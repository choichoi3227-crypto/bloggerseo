...
// CACHE_RESERVE_KV — "연산 결과"(pageCtx 등 파생 메타데이터) 캐시 TTL
// ※ HTML 본문/최종 응답 바이트는 절대 이 KV에 저장하지 않음
...
// CACHE_RESERVE_KV 전용 키 — 입력 HTML 해시 기반 (콘텐츠 변경 시 자동 무효화)
...
async function handleFetch(request, env, ctx) {
  ...
  if (path === '/__health') {
    return new Response(JSON.stringify({
      ok: true, ts: Date.now(), wasm: wasmCore._lastBackend,
      bindings: {
        SLUG_KV: !!env.SLUG_KV,
        CNAME_KV: !!env.CNAME_KV,
        CACHE_RESERVE_KV: !!env.CACHE_RESERVE_KV,
      },
    }), {
...
async function purgeAll(env) {
  ...
  return new Response(JSON.stringify({ purged: deleted, computeCachePurged }), {
    status: 200, headers: { 'content-type': 'application/json', 'cache-control': 'no-store' },
  });
}
...
// ─────────────────────────────────────────────────────────────────
// 페이지 컨텍스트 — CACHE_RESERVE_KV 캐시 래퍼
// ─────────────────────────────────────────────────────────────────
// 캐시 대상: pageCtx(JSON 메타데이터)만. HTML 본문/응답 바이트는 절대 저장하지 않음.
// 캐시 키: origin raw HTML의 FNV 해시 → 글 내용이 바뀌면 키가 바뀌어 자동 무효화.
// 캐시 읽기/쓰기는 전부 실패해도 무시하고 직접 계산으로 폴백 — 캐시는 순수 가속 레이어.
async function getOrComputePageContext(html, url, host, path, env, ctx, metrics) {
  let cacheKey = null;

  if (env.CACHE_RESERVE_KV) {
    try {
      const hash = await wasmCore.fnv1a32Hex(html.length > 16384 ? html.slice(0, 16384) : html);
      ...
      // 캐시 조회 실패 — 무시하고 아래에서 직접 계산
      cacheKey = null;
    }
  }

  const ctxData = await extractPageContext(html, url);

  if (cacheKey) {
    // 쓰기는 응답 경로를 막지 않도록 비동기로 분리 — 실패해도 무시
    ctx.waitUntil(
      env.CACHE_RESERVE_KV.put(cacheKey, JSON.stringify(ctxData), {
...
async function transformHtml(html, ctx, url, env) {
  ...
  // ② 성능 최적화 (가장 먼저 — head에 주입, CSS 없이 JS/HTML 속성 기반)
  ...
  // ⑤ 접근성 (markup-only — 시각적 스타일/레이아웃 변경 없음)
  o = safeTransform(o, injectLangAttribute);

  // ⑥ 본문 보강 (순서 중요: alt 보완 → 그 alt를 참조하는 단계들)
  o = safeTransform(o, h => boostExternalLinks(h));
  o = safeTransform(o, h => injectImageAlts(h, ctx));
  o = safeTransform(o, h => injectImagePerfAttrs(h, ctx));
  o = safeTransform(o, injectLinkAriaLabels);
...
// 접근성: <html> lang 속성 누락 시에만 보완 (기존 lang 있으면 절대 건드리지 않음)
function injectLangAttribute(html) {
  if (/<html[^>]+lang=["'][^"']+["']/i.test(html)) return html;
  return html.replace(/<html([^>]*)>/i, (match, attrs) => {
    // class 등 기존 속성 보존, lang만 추가
...
// 성능: <img>에 loading="lazy" / decoding="async" 누락 시에만 보완
// LCP 후보(본문 첫 이미지·og:image와 동일 src)는 lazy 제외 — 초기 렌더 지연 방지
function injectImagePerfAttrs(html, ctx) {
  let firstImgSeen = false;
  return html.replace(/<img([^>]+)>/gi, (match, attrs) => {
    const isLcpCandidate = !firstImgSeen && !!ctx.imageUrl && attrs.includes(ctx.imageUrl);
    if (!firstImgSeen) firstImgSeen = true;

    let out = attrs;
    if (!/loading=/i.test(out) && !isLcpCandidate) {
      out += ' loading="lazy"';
    }
    if (!/decoding=/i.test(out)) {
      out += ' decoding="async"';
    }
    return out === attrs ? match : `<img${out}>`;
  });
}

// 접근성: 텍스트 없이 이미지만 담긴 <a> 태그에 aria-label 보완 (이미지 alt 기반)
// 이미 텍스트나 aria-label이 있으면 건드리지 않음 — 스크린리더 안내 누락만 보완
function injectLinkAriaLabels(html) {
  return html.replace(/<a([^>]*)>([\s\S]*?)<\/a>/gi, (match, attrs, inner) => {
    if (/aria-label=/i.test(attrs)) return match;
    const textOnly = inner.replace(/<[^>]+>/g, '').trim();
    if (textOnly) return match; // 텍스트 콘텐츠가 이미 있으면 보완 불필요
    const imgAlt = (inner.match(/<img[^>]+alt=["']([^"']+)["']/i) || [])[1];
    if (!imgAlt) return match; // 라벨로 쓸 단서가 없으면 임의로 만들지 않음
    return `<a${attrs} aria-label="${escapeAttr(imgAlt)}">${inner}</a>`;
  });
}
